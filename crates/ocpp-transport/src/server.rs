...
impl OcppServer {
    ...
    /// Ask a connected charge point to download and install firmware from
    ...
    /// ([`examples/v16/central_system.py`](https://github.com/mobilityhouse/ocpp/blob/master/examples/v16/central_system.py)).
    ///
    /// `location` is the URL the CP fetches the firmware from; `retrieve_date`
    /// is the earliest time it may start. `retries` / `retry_interval` bound the
    /// CP's download attempts — both optional, matching the spec. The
    /// [`UpdateFirmwareResponse`] is empty per the spec (it carries no status):
    /// the CP reports progress out-of-band via `FirmwareStatusNotification`, so
    /// a successful resolution means only that the CP accepted the command.
    /// Errors propagate from [`call`](Self::call) (e.g.
    /// [`OcppError::CpNotConnected`], [`OcppError::Timeout`]).
