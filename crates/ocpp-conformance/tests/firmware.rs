...
//! helper, asserting the CP not only acknowledges the command but **actually
//! runs** the simulated download/install off the inbound-CALL path, emitting
//! `FirmwareStatusNotification` `Downloading` → `Downloaded` → `Installing` →
//! `Installed`:
//!
//!   1. `update_firmware(...)` resolves (the conf is empty per the spec), and
//!      the CSMS then observes Downloading → Downloaded → Installing →
//!      Installed, in order.
...
#[tokio::test]
async fn csms_update_firmware_drives_install_state_machine() {
    let cp_id = "CP_FW_01";
    ...
    // 1. UpdateFirmware resolves (empty conf), and the CP runs the update.
    server
        .update_firmware(
            cp_id,
    ...
    // The simulated update reports each step, in order.
